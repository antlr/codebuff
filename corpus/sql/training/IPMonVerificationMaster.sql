       ...
       AND t.server_name NOT IN (SELECT DISTINCT l.address
                                 FROM   ipmongroups g
                                        ...
                                        INNER JOIN t_server_type_assoc tsta
                                          ON t.server_id = tsta.server_id
                                 WHERE
           l.name LIKE '%SQL Server Agent (MSSQLSERVER)%'
           AND t.environment_id = 0
           AND tsta.type_id = 1 --DB Server
           AND g.groupname IN ( 'Prod SQL Services' )
           AND t.active = 1)
       ...
       AND t.server_name NOT IN (SELECT DISTINCT l.address
                                 FROM   ipmongroups g
                                        ...
                                        INNER JOIN t_server_type_assoc tsta
                                          ON t.server_id = tsta.server_id
                                 WHERE
           l.name LIKE '%SQL Server Integration Services%'
           AND t.environment_id = 0
           AND tsta.type_id = 1 --DB Server
           AND g.groupname IN ( 'Prod SQL Services' )
           AND t.active = 1)
...
WHERE  t.active = 1
       ...
       AND t.server_name NOT IN ('PWEBUSB20', 'PWEBUSB21', 'XWEBUTIL12', 'XWEBUTIL13'
								, 'EWEBPROD1', 'PWEBBO20', 'PWEBBO21', 'PWEBMET20', 'PWEBMET21')
       AND t.server_name NOT IN (SELECT DISTINCT l.address
                                 FROM   ipmongroups g
                                        INNER JOIN ipmongroupmembers m
                                          ON g.groupid = m.groupid
