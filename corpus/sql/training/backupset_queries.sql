...
FROM   msdb_dbo_backupmediafamily 
   INNER JOIN msdb_dbo_backupset ON msdb_dbg_backupmediafamily_media_set_id = msdb_dbo_backupset_media_set_id 
...
FROM
   ...
   FROM    msdb_dbg_backupmediafamily 
       INNER JOIN msdb_dbo_backupset ON msdb_dbg_backupmediafamily_media_set_id = msdb_dbo_backupset_media_set_id 
...
HAVING      (MAX(msdb_dbo_backupset_backup_finish_date) < DATEADD(hh, - 24, GETDATE())) 
...
FROM
   master_dbo_sysdatabases LEFT JOIN msdb_dbo_backupset
       ON master_dbo_sysdatabases.name  = msdb_dbo_backupset_database_name
WHERE msdb_dbo_backupset_database_name IS NULL AND master_dbo_sysdatabases.name <> 'tempdb'
ORDER BY 
   msdb_dbo_backupset_database_name
