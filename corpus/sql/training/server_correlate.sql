...
where database_name = '" & db_name & "' order by 1,3,2


select s.server_name from
	dbo.t_server s INNER JOIN
...
where s.server_name NOT in ( 'opsdb.dexma.com', 'opsdb.demo.dexma.com', 'impopsdb.dexma.com', '(local)', 'OPSFH.DEXMA.COM') 
AND active = '1' order by server_name

select server_name from t_server where server_id IN (select server_id from t_sched_task) order by server_name

SELECT s.server_id, s.server_name, st.account, st.app_name, st.working_dir, st.comment, st.creator, st.trigger_string 
from	dbo.t_server s INNER JOIN 
	dbo.t_sched_task st ON st.server_id = s.server_id
where s.server_name = 'alya'
order by s.server_name


SELECT s.server_name, e.environment_name, ws.*
from	dbo.t_websites ws INNER JOIN
	dbo.t_server s ON ws.server_id = s.server_id INNER JOIN
