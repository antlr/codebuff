...
FROM dbo.t_server s INNER JOIN
    dbo.t_proc_controller_assoc pca ON pca.server_id = s.server_id
    INNER JOIN
...
SELECT DISTINCT
    ...
    , cqa.assoc_id
FROM dbo.t_server s INNER JOIN
    dbo.t_component_queue_assoc cqa ON cqa.server_id = s.server_id
    INNER JOIN
    dbo.t_component c ON c.component_id = cqa.component_id
    INNER JOIN
...
SELECT DISTINCT
    ...
    , qsa.LastUpdate
FROM dbo.t_server s INNER JOIN
    dbo.t_queue_server_assoc qsa ON qsa.server_id = s.server_id
    INNER JOIN
...
FROM dbo.t_queue q INNER JOIN
    dbo.t_queue_server_assoc qsa ON q.queue_id = qsa.queue_id
    INNER JOIN
...
FROM dbo.t_server s LEFT OUTER JOIN
    dbo.t_server_type_assoc sta ON sta.server_id = s.server_id
    LEFT OUTER JOIN
