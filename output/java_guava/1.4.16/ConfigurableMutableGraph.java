...
final class ConfigurableMutableGraph<N> extends AbstractConfigurableGraph<N> implements MutableGraph<N> {
  ...
  public boolean addEdge(N node1, N node2) {
    ...
    if (containsN1 && containsN2
        && nodeConnections.get(node1).successors().contains(node2)) {
      return false;
    }
    if (!containsN1) {
      addNode(node1);
  ...
  private NodeAdjacencies<N> newNodeConnections() {
    return isDirected() ? DirectedNodeAdjacencies.<N>of() : UndirectedNodeAdjacencies.<N>of();
  }
}