...
class ComputingConcurrentHashMap<K, V> extends MapMakerInternalMap<K, V> {
  ...
  private static final class ComputingValueReference<K, V> implements ValueReference<K, V> {
    ...
    V compute(K key, int hash) throws ExecutionException {
      ...
      setValueReference(
                 new ComputedReference<K, V>(value));
      return value;
    }
  ...
  static final class ComputingSerializationProxy<K, V> extends AbstractSerializationProxy<K, V> {
    ...
    ComputingSerializationProxy(Strength keyStrength, Strength valueStrength, Equivalence<Object> keyEquivalence, Equivalence<Object> valueEquivalence, int concurrencyLevel, ConcurrentMap<K, V> delegate, Function<? super K, ? extends V> computingFunction) {
