    ...
    Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
      ...
      if (!upperBoundWindow.hasLowerBound()) {
        ...
        if (lowerEntry == null) {
          ...
          backingItr =
             rangesByLowerBound.tailMap(upperBoundWindow.lowerEndpoint(), true).values().iterator();
        }
      }
...
complementLowerBoundWindow.lowerEndpoint(), complementLowerBoundWindow.lowerBoundType() == BoundType.CLOSED).values();
      } else {
  ...
  private static final class SubRangeSetRangesByLowerBound<C extends Comparable<?>> extends AbstractNavigableMap<Cut<C>, Range<C>> {
      ...
      if (key instanceof Cut) {
        try {
          ...
          if (!lowerBoundWindow.contains(cut)
            ...
            if (candidate != null
                && candidate.upperBound.compareTo(restriction.lowerBound) > 0) {
    ...
    Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
      ...
      if (lowerBoundWindow.upperBound.isLessThan(restriction.lowerBound)) {
        ...
        completeRangeItr = rangesByLowerBound.tailMap(
          lowerBoundWindow.lowerBound.endpoint(), lowerBoundWindow.lowerBoundType() == BoundType.CLOSED).values().iterator();
      }
    ...
    Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
      ...
      final Iterator<Range<C>> completeRangeItr = rangesByLowerBound.headMap(
        upperBoundOnLowerBounds.endpoint(), upperBoundOnLowerBounds.typeAsUpperBound() == BoundType.CLOSED).descendingMap().values().iterator();
      return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
        @Override
        protected Entry<Cut<C>, Range<C>> computeNext() {
          if (!completeRangeItr.hasNext()) {
