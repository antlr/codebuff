...
public final class ImmutableGraph<N> extends AbstractConfigurableGraph<N> {
  ...
  private static <N> NodeAdjacencies<N> nodeConnectionsOf(Graph<N> graph, N node) {
    return graph.isDirected() ? DirectedNodeAdjacencies.ofImmutable(
      createAdjacencyMap(graph, node),
      graph.predecessors(node).size(),
      graph.successors(node).size()) : UndirectedNodeAdjacencies.ofImmutable(graph.adjacentNodes(node));
  }
