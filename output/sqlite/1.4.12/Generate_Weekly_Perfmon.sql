...
FROM
    (
        SELECT
            ...
            , (sum(cd.CounterValue) - min(cd.CounterValue) - max(cd.CounterValue)) / cast(count(*) - 2
            AS FLOAT) AS MeanAvgValue
...
FROM
    (
        SELECT
            ...
            , (sum(cdd.MeanAvgValue) - min(cdd.MeanAvgValue) - max(cdd.MeanAvgValue)) / cast(count(*) - 2
            AS FLOAT) AS MeanAvgValue
