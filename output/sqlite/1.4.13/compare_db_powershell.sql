...
SELECT
    @EmployeeList = COALESCE(@EmployeeList + ', ', '') + CAST(Emp_UniqueID AS VARCHAR(5))
...
SELECT
    @dblistStage = COALESCE(@dblistStage + ', ', '') + name
FROM sys.databases
...
SELECT
    @ServerList = COALESCE(@ServerList + ', ', '') + RTRIM(LTRIM([server_name]))
