...
WHERE t.active = 1
    ...
    (
                            ...
                            FROM ipmongroups g INNER JOIN
                                 ipmongroupmembers m ON g.groupid = m.groupid
                                 INNER JOIN
                                 ipmonmonitors l ON m.monitorid = l.monitorid
                                 INNER JOIN
                                 t_server t ON l.address = t.server_name
                                 INNER JOIN
                                 t_server_type_assoc tsta ON t.server_id = tsta.server_id
                            WHERE l.name LIKE '%PING%' AND t.environment_id = 0
                                  AND tsta.type_id IN ('1', '2', '3', '4', '5', '6', '7', '8', '9', '15', '22', '24', '26', '29')
...
WHERE t.active = 1
    ...
    (
          ...
          FROM ipmongroups g INNER JOIN
               ...
               INNER JOIN
               t_server_type_assoc tsta ON t.server_id = tsta.server_id
          WHERE l.name LIKE '%Task Scheduler%' AND t.environment_id = 0
                AND tsta.type_id IN ('1', '2', '3', '4', '5', '6', '7', '8', '9', '15', '22', '24', '26', '29')
                AND g.groupname IN ('Prod O/S Services')
                AND t.active = 1
      )
...
FROM t_server t INNER JOIN
     t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
      AND tsta.type_id = 1 --DB Server AND t.environment_id = 0
      -- exclude servers that don't meet the requirements
      AND t.server_name NOT IN
          ( 'XSQLUTIL11')
      AND t.server_name NOT IN
...
--where l.name like '%IIS Admin Service%'
                                     WHERE l.name LIKE '%IIS Admin%' AND t.environment_id = 0
                                           AND tsta.type_id IN ('2', '4', '30', '31')
          --Web, Connection, FTP, Internal FTP server
...
FROM t_server t INNER JOIN
     t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1 AND tsta.type_id IN ('2') --Web Server
      AND t.environment_id = 0
      -- exclude servers that don't meet the requirements
      AND t.server_name NOT IN
    ...
    (
          ...
          FROM ipmongroups g INNER JOIN
               ...
               INNER JOIN
               t_server_type_assoc tsta ON t.server_id = tsta.server_id
          WHERE l.name LIKE '%World Wide Web%' AND t.environment_id = 0
...
WHERE t.active = 1
    ...
    (
          ...
          FROM ipmongroups g INNER JOIN
               ...
               INNER JOIN
               t_server_type_assoc tsta ON t.server_id = tsta.server_id
          WHERE l.name LIKE '%ftp' AND t.environment_id = 0 AND tsta.type_id IN ('4', '30', '31')
...
FROM t_server t INNER JOIN
     t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1 AND tsta.type_id IN ('5')
      AND t.environment_id = 0
      ...
      AND t.server_name NOT IN(
                                     ...
                                     FROM ipmongroups g INNER JOIN
                                          ...
                                          INNER JOIN
                                          t_server_type_assoc tsta ON t.server_id = tsta.server_id
                                     WHERE l.name LIKE '%File Access%' AND t.environment_id = 0
