...
FROM sysobjects tab
    ...
    LEFT OUTER JOIN sysobjects obj ON com.id = obj.id
      , systypes typ
WHERE tab.id = col.id AND tab.xtype = 'U' AND tab.name <> 'dtproperties'
      AND
...
WHERE obj.xtype = 'C'
      AND
      com.id = obj.id AND tab.id = obj.parent_obj
      AND
...
WHERE ind.status & 800
      = 800
      AND
      ind.id = tab.id AND idk.id = tab.id AND idk.indid = ind.indid
      AND
      tab.name <> 'dtproperties'
ORDER BY 1, 2, 4
...
WHERE NOT(ind.status & 800 = 800)
      ...
      AND
      idk.id = tab.id AND idk.indid = ind.indid AND tab.xtype = 'U'
      AND
      tab.id = ind.id
      AND
      tab.name <> 'dtproperties'
