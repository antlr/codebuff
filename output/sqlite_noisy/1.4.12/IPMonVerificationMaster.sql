...
AND t.environment_id = 0 AND tsta.type_id IN ('1', '2', '3', '4', '5', '6', '7', '8', '9', '15', '22', '24', '26', '29')
                         AND g.groupname IN ('Prod O/S Services')
    AND t.active = 1)
UNION ALL
---------------------------------------------------------------------------------------------------
...
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements AND t.server_name NOT IN ('XSQLUTIL11')
AND t.server_name NOT IN(SELECT DISTINCT l.address FROM    ipmongroups g
                         INNER JOIN
                                                           ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN
                                                           ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN
...
AND tsta.type_id = 1 --DB Server
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements AND t.server_name NOT IN ('XSQLUTIL11', 'EWEBPROD1')
AND t.server_name NOT IN(SELECT DISTINCT l.address FROM    ipmongroups g
                         INNER JOIN
                                                           ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN
                                                           ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN
                                                           t_server t ON l.address = t.server_name INNER JOIN
                                                           t_server_type_assoc tsta ON t.server_id = tsta.server_id
                         WHERE l.name LIKE '%SQL Server Agent (MSSQLSERVER)%' AND
                               t.environment_id = 0
AND tsta.type_id = 1 --DB Server
                         AND g.groupname IN ('Prod SQL Services')
    AND t.active = 1)
...
FROM    t_server t
    INNER JOIN
        t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
AND tsta.type_id IN ('4', '30', '31') --Connection, FTP or Internal FTP AND
      t.environment_id = 0
AND t.server_name NOT IN(SELECT DISTINCT l.address FROM    ipmongroups g
                         INNER JOIN
                                                           ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN
