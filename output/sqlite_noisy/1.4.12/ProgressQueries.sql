...
, DATEDIFF(ms, LoadStageDBStartDate, LoadStageDBEndDate), 0), 114) AS StageLoadTime
               ...
               , LoadReportDBEndDate
               , CONVERT(VARCHAR(12), DATEADD(ms
, DATEDIFF(ms, LoadReportDBStartDate, LoadReportDBEndDate), 0), 114) AS ReportLoadTime
...
SELECT  CASE WHEN CAST(Beta AS VARCHAR) IS NULL THEN 'Grand Total' ELSE CAST(Beta AS VARCHAR) END AS Beta,
        CASE WHEN SourceDB IS NULL THEN 'Beta Group Total' ELSE SourceDB END AS SourceDB
      --, LoadStageDBStartDate
      --, LoadStageDBEndDate
                                                                                                         , CONVERT(VARCHAR(12), DATEADD(ms
, SUM(DATEDIFF(ms, LoadStageDBStartDate, LoadStageDBEndDate)), 0), 114) AS StageLoadTime
      --, LoadReportDBStartDate
      --, LoadReportDBEndDate
