...
group by ServerName, DBName, SQLStatement, IndexType, FragPercent having count(*) > 1
        ) b
...
FROM   sys.dm_db_index_usage_stats
    JOIN sys.indexes
        ON sys.dm_db_index_usage_stats.object_id = sys.indexes.object_id AND sys.dm_db_index_usage_stats.index_id = sys.indexes.index_id AND sys.indexes.name NOT LIKE 'PK%'
...
WHERE sys.dm_db_index_usage_stats.database_id = DB_ID()
    ...
    AND user_lookups = 0
AND user_seeks = 0
AND sys.dm_db_index_usage_stats.index_id NOT IN ( 0
, 1)
