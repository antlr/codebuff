/* Get tables */
SELECT tab.name table_name FROM sysobjects tab
WHERE tab.xtype = 'U'
...
SELECT tab.name table_name
    ...
    , CASE WHEN col.isnullable = 1 THEN 'Y' ELSE 'N'
      END is_nullable
    , CASE WHEN col.status & 80 = 80 THEN 'Y' ELSE 'N'
      END is_identity
...
SELECT tab.name table_name, obj.name constraint_name, com.text condition
       FROM   sysobjects tab,
       syscomments com,
       sysobjects obj
WHERE obj.xtype = 'C'
AND com.id = obj.id AND tab.id = obj.parent_obj AND tab.name <> 'dtproperties'
ORDER BY 1,2
    ...
    , parent_column parent_column
       FROM (
            SELECT tab1.name child_table
                ...
                , CASE col1.colid WHEN ref.fkey1 THEN 1
...
WHEN ref.fkey16 THEN 16 END child_pos
                , tab2.name parent_table
                , col2.name parent_column
                , ref.constid constraint_id
                , CASE col2.colid WHEN ref.rkey1 THEN 1
...
WHEN ref.rkey16 THEN 16 END parent_pos
            FROM syscolumns col1, sysobjects tab1, syscolumns col2, sysobjects tab2, sysreferences ref
            WHERE col1.id = ref.fkeyid AND tab1.id = col1.id AND col2.id = ref.rkeyid AND tab2.id = col2.id AND col1.colid IN (
ref.fkey1,
ref.fkey2,
ref.fkey3,
ref.fkey4,
...
SELECT tab.name table_name
    , ind.name index_name
    , INDEX_COL(tab.name, ind.indid, idk.keyno) column_name
    , CASE WHEN ind.status & 2 = 2 THEN 'Y' ELSE 'N'
      END is_unique
FROM   sysindexes ind,
       sysindexkeys idk,
       sysobjects tab
WHERE NOT(ind.status & 800 = 800)
      ...
      AND idk.id = tab.id AND idk.indid = ind.indid AND tab.xtype = 'U'
      AND tab.id = ind.id AND tab.name <> 'dtproperties'
ORDER BY 1,2