...
select server_id, CountersID, MinValue, MeanAvgValue, MaxValue
                , Day, GetDate() AS LastUpdate
FROM
    (
    SELECT s.server_id
        , c.CountersID
        , min(cd.CounterValue) AS MinValue
        , (sum(cd.CounterValue) -min(cd.CounterValue) -max(cd.CounterValue))/cast(count(*) - 2 as float) AS MeanAvgValue
...
--where  convert(char(10), CounterDateTime, 101) = (GetDate() - 6) GROUP BY s.server_id, c.CountersID, convert(char(10), CounterDateTime, 101)
    ) AS f
...
SELECT s.server_id
    ...
    , (sum(cdd.MeanAvgValue) -min(cdd.MeanAvgValue) -max(cdd.MeanAvgValue))/cast(count(*) - 2 as float) AS MeanAvgValue
...
where  convert(char(10), Date, 101) > GetDate() - 3 GROUP BY s.server_id, c.CountersID, convert(char(10), Date, 101)
) AS f
