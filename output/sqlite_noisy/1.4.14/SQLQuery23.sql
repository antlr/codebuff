SELECT SourceDB
    ...
    , TaskName
        --, ErrorMessage
        --, ErrorDateTime
    , (SELECT ErrorDateTime WHERE ErrorMessage LIKE '%Validation phase is beginning.%') AS TaskStartTime
    , (SELECT ErrorDateTime WHERE ErrorMessage LIKE '%Cleanup phase is beginning.%') AS TaskEndTime
    , CONVERT(NVARCHAR(12), DATEADD(ms, DATEDIFF(ms, (SELECT ErrorDateTime WHERE ErrorMessage LIKE '%Validation phase is beginning.%'), (SELECT ErrorDateTime WHERE ErrorMessage LIKE '%Cleanup phase is beginning.%')), 0), 114) AS TaskTimeTaken
        --, * 
...
ORDER BY SourceDB, TaskName, ErrorDateTime ASC
-----------------------

SELECT SourceServer, SourceDB, DestServer, DestDB, PackageName, TaskName, ErrorMessage, ErrorDateTime
FROM dbo.DMartComponentLogging
                   WHERE DATEPART(day, ErrorDateTime) = DATEPART(day, GetDate())
AND DATEPART(month, ErrorDateTime) = DATEPART(month, GetDate())
AND DATEPART(year, ErrorDateTime) = DATEPART(year, GetDate())
...
SELECT DISTINCT dmcl.ClientID,
       dmcl.SourceServer
                ...
                , dmcl.ErrorMessage
                , dmcl.ErrorDateTime
                , (SELECT dmcl.ErrorDateTime WHERE dmcl.ErrorMessage LIKE '%Validation phase is beginning.%') AS TaskStartTime
...
FROM
    dbo.DMartComponentLogging dmcl
