...
select distinct name from t_proc_controller
order by name
...
from   dbo.t_server s
    INNER JOIN dbo.t_component_queue_assoc cqa
    ON cqa.server_id = s.server_id
    INNER JOIN dbo.t_component c
    ON c.component_id = cqa.component_id
    INNER JOIN dbo.t_queue q
...
from   dbo.t_server s
    ...
    LEFT OUTER JOIN dbo.t_environment e
