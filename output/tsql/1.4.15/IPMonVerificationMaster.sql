...
WHERE t.active = 1
      ...
      WHERE l.name LIKE '%Remote Procedure Call%'
            ...
            ('1', '2', '3', '4', '5', '6', '7', '8', '9', '15', '22', '24', '26', '29')
            AND
            g.groupname IN ('Prod O/S Services')
            AND
            t.active = 1)
UNION ALL
---------------------------------------------------------------------------------------------------
...
WHERE t.active = 1
     ...
     t.environment_id = 0
      -- exclude servers that don't meet the requirements
     AND
     t.server_name NOT IN ('XSQLUTIL11')
     AND
     t.server_name NOT IN (
     SELECT DISTINCT l.address
     FROM ipmongroups g
     ...
     WHERE l.name LIKE '%SQL Server (MSSQLSERVER)%'
           ...
           tsta.type_id = 1 --DB Server
           AND
           g.groupname IN ('Prod SQL Services')
           AND
           t.active = 1)
UNION ALL
---------------------------------------------------------------------------------------------------
...
WHERE t.active = 1
     ...
     t.environment_id = 0
      -- exclude servers that don't meet the requirements
     AND
     t.server_name NOT LIKE 'PWEBSVC%'
...
--where l.name like '%IIS Admin Service%'
     WHERE l.name LIKE '%IIS Admin%'
          ...
          --Web, Connection, FTP, Internal FTP server
           AND
           g.groupname IN ('Prod IIS Services')
           AND
           t.active = 1)
UNION ALL
---------------------------------------------------------------------------------------------------
...
WHERE t.active = 1
     ...
     WHERE l.name LIKE '%File Access%'
           ...
           tsta.type_id IN ('5')
           AND
           g.groupname IN ('Prod File Access')
           AND
           t.active = 1)
--UNION ALL
