...
WHERE DATEPART(day, ErrorDateTime) = DATEPART(day, GetDate())
      ...
      AND
      ErrorMessage NOT LIKE '%Pre-Execute phase is beginning.  %'
      AND
      ErrorMessage NOT LIKE '%Prepare for Execute phase is beginning.  %'
      AND
      ErrorMessage NOT LIKE '%The final commit for the data insertion%'
      AND
      ErrorMessage NOT LIKE '%The buffer manager detected%'
      AND
      ErrorMessage NOT LIKE '%Post Execute phase is beginning.%'
      AND
      ErrorMessage NOT LIKE '%Execute phase is beginning.%'
      AND
      ErrorMessage NOT LIKE '%component%wrote%'
...
FROM dbo.DMartComponentLogging dmcl
    JOIN dbo.DMartComponentLogging dmcl2 ON dmcl.TaskName = dmcl2.TaskName AND dmcl.SourceServer = dmcl2.SourceServer AND dmcl.ClientID = dmcl2.ClientID
WHERE DATEPART(day, dmcl.ErrorDateTime) = DATEPART(day, GetDate())
      ...
      AND
      dmcl.ErrorMessage NOT LIKE '%Pre-Execute phase is beginning.  %'
      AND
      dmcl.ErrorMessage NOT LIKE '%Prepare for Execute phase is beginning.  %'
      AND
      dmcl.ErrorMessage NOT LIKE '%The final commit for the data insertion%'
      AND
      dmcl.ErrorMessage NOT LIKE '%The buffer manager detected%'
      AND
      dmcl.ErrorMessage NOT LIKE '%Post Execute phase is beginning.%'
      AND
      dmcl.ErrorMessage NOT LIKE '%Execute phase is beginning.%'
      AND
      dmcl.ErrorMessage NOT LIKE '%component%wrote%'
