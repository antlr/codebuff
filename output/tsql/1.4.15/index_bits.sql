...
FROM SQLIndexRebuilds a
    JOIN (
             ...
             HAVING count(*) > 1
         ) b ON a.ServerName = b.ServerName AND a.DBName = b.DBName AND a.SQLStatement = b.SQLStatement AND a.IndexType = b.IndexType AND a.FragPercent = b.FragPercent AND a.seq_num < b.max_seq_num
...
FROM sys.dm_db_index_usage_stats
    JOIN
    sys.indexes ON sys.dm_db_index_usage_stats.object_id = sys.indexes.object_id AND sys.dm_db_index_usage_stats.index_id = sys.indexes.index_id AND sys.indexes.name NOT LIKE 'PK%'
                   AND
