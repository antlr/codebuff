        ...
        (
            SELECT
                ...
                , convert(char(10), CounterDateTime,
                          101) AS Day
            FROM Counters c
                INNER JOIN CountersData cd ON c.CountersID = cd.CountersID
                INNER JOIN
...
--where  convert(char(10), CounterDateTime, 101) = (GetDate() - 6)
            GROUP BY s.server_id, c.CountersID, convert(char(10), CounterDateTime,
                                                        101)
        ) AS f
    ...
    (
        SELECT
            ...
            , convert(char(10), Date,
                      101) AS Day
        FROM Counters c
            INNER JOIN CountersDataDaily cdd ON c.CountersID = cdd.CountersID
            INNER JOIN
            newton.Statusdbot_server s ON s.server_id = cdd.ServerID
        WHERE convert(char(10), Date,
                      101) > GetDate() - 3
        GROUP BY s.server_id, c.CountersID, convert(char(10), Date,
                                                    101)
    ) AS f
