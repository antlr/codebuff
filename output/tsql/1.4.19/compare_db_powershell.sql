...
FROM sys.databases
WHERE
    name LIKE 'DMart%Data' AND name NOT LIKE '%Template%'
GROUP BY name
SELECT @dblistStage = COALESCE(@dblistStage + ', ', '') + name
FROM sys.databases
WHERE
    name LIKE 'DMart%Stage' AND name NOT LIKE '%Template%'
GROUP BY name
SELECT @cmd2 =
...
FROM sys.databases
WHERE
    name LIKE 'DMart%CDC%Data' AND name NOT LIKE '%Template%'
GROUP BY name
SELECT @CDCcmd =
...
SELECT @cmd =
       'E:\Dexma\powershell_bits\Compare-DbamaintSchema.ps1 '+ '-ServerList ' + @ServerList + ' -Column -Log'
...
FROM sys.databases
WHERE
    name LIKE '%SMC' AND name NOT LIKE '%Test%'
GROUP BY name
SELECT @dblistStage = COALESCE(@dblistStage + ', ', '') + name
FROM sys.databases
WHERE
...
SELECT @cmd3 = 'E:\Dexma\powershell_bits\Compare-DMartSchema2.ps1 -SqlServerOne ' +
               'ISQLDEV610 -FirstDatabase SMCCurrent -SqlServerTwo ' + @SERVERNAME + ' -DatabaseList ' + @dblistStage + ' -Column -Log'