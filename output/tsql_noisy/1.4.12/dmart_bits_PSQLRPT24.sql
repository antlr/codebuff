...
AND CONVERT(DATE, EndTimeExtract) < CONVERT(DATE, GETDATE()) THEN Status ELSE NULL END ) AS OldStatus4
                                                     , COUNT ( CASE WHEN Status = 0 THEN Status ELSE NULL END ) AS Status0
                                                     , COUNT ( CASE WHEN Status = 1 THEN Status ELSE NULL END ) AS Status1
                                                     , COUNT ( CASE WHEN Status = 2 THEN Status ELSE NULL END ) AS Status2
                                                     , COUNT ( CASE WHEN Status = 3 THEN Status ELSE NULL END ) AS Status3
                                                     , COUNT ( CASE WHEN Status = 4
AND DATEPART(DAY,EndTimeExtract) = DATEPART(DAY,GETDATE()) THEN Status ELSE NULL END ) AS Status4
FROM    dbo.ClientConnectionCDC
GROUP BY SSISInstanceID
) AS StatusMatrix
GROUP BY InstanceID
...
FROM
(
SELECT ClientID, [dbo].[ClientConnectionCDC].[CDCExtractDB]
               ,[Status]
               , Beta
...
SET SSISinstanceID = CASE SourceServer WHEN 'PSQLDLS30' THEN 0 WHEN 'PSQLDLS31' THEN 1 WHEN 'PSQLDLS32' THEN 2 WHEN 'PSQLDLS33' THEN 3 WHEN 'PSQLDLS34' THEN 4 WHEN 'PSQLDLS35' THEN 5 WHEN 'PSQLRPT24' THEN 5 -- DMart Template END
