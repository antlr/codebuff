SELECT DISTINCT t.server_name, t.server_id, 'PING' AS missingmonitors
FROM t_server t
...
WHERE t.active = 1
       ...
       -- exclude servers that don't meet the requirements
                 AND t.server_name NOT IN ('XWEBUTIL12','XWEBUTIL13','EWEBPROD1','PWEBSVC20','PWEBUSB20','XSQLUTIL11')
                 AND t.server_name NOT IN (SELECT DISTINCT l.address FROM ipmongroups g
                                               INNER JOIN ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN t_server t ON l.address = t.server_name INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
...
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements
AND t.server_name NOT IN ('XWEBUTIL12','XWEBUTIL13','EWEBPROD1','XSQLUTIL11')
AND t.server_name NOT IN (SELECT DISTINCT l.address FROM ipmongroups g
                              INNER JOIN ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN t_server t ON l.address = t.server_name INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
...
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements
AND t.server_name NOT IN ('XWEBUTIL12','XWEBUTIL13','XSQLUTIL11','EWEBPROD1','PVM400')
AND t.server_name NOT IN (SELECT DISTINCT l.address FROM ipmongroups g
                              INNER JOIN ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN t_server t ON l.address = t.server_name INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
...
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'Message Queuing Service' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
...
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'SQL Service' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
...
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'SQL Agent Service' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
AND tsta.type_id = 1 --DB Server
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements
AND t.server_name NOT IN ('XSQLUTIL11','EWEBPROD1')
AND t.server_name NOT IN (SELECT DISTINCT l.address FROM ipmongroups g
                              INNER JOIN ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN t_server t ON l.address = t.server_name INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
...
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'SQL Integration Service' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
AND tsta.type_id = 1 --DB Server
...
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'IIS Admin Service' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
AND tsta.type_id IN ('2','4','30','31') --Web, Connection, FTP, Internal FTP server
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements
AND t.server_name NOT LIKE 'PWEBSVC%'
       -- 2008 R2 servers do not have the IIS Admin service
AND t.server_name NOT IN ('PWEBUSB20','PWEBUSB21','XWEBUTIL12','XWEBUTIL13','EWEBPROD1','PWEBBO20','PWEBBO21','PWEBMET20','PWEBMET21')
AND t.server_name NOT IN (SELECT DISTINCT l.address FROM ipmongroups g
                              INNER JOIN ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN t_server t ON l.address = t.server_name INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
...
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'W3SVC Service' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
...
AND t.environment_id = 0
       -- exclude servers that don't meet the requirements
AND t.server_name NOT IN ('XWEBUTIL12','XWEBUTIL13','EWEBPROD1')
AND t.server_name NOT IN (SELECT DISTINCT l.address FROM ipmongroups g
                              INNER JOIN ipmongroupmembers m ON g.groupid = m.groupid INNER JOIN ipmonmonitors l ON m.monitorid = l.monitorid INNER JOIN t_server t ON l.address = t.server_name INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
                          WHERE l.name LIKE '%World Wide Web%'
                                          ...
                                          AND t.active = 1) EXCEPT
                                                            SELECT t.server_name, t.server_id, 'W3SVC Service' AS missingmonitors --exclude connection servers
                                                            FROM t_server t
                                                                INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
                                                            WHERE tsta.type_id IN ('4')
UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'FTP Port Access' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
...
--UNION ALL
---------------------------------------------------------------------------------------------------
SELECT DISTINCT t.server_name, t.server_id, 'File Access' AS missingmonitors
FROM t_server t
INNER JOIN t_server_type_assoc tsta ON t.server_id = tsta.server_id
WHERE t.active = 1
