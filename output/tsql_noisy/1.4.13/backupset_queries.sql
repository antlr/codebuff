...
SELECT CONVERT(CHAR(100), SERVERPROPERTY('Servername')) AS Server
    ...
    , CASE msdb_backupset.type WHEN 'D' THEN 'Database'
...
FROM   msdb_dbg_backupmediafamily INNER JOIN msdb_dbo_backupset
                                  ON msdb_dbg_backupmediafamily_media_set_id = msdb_dbo_backupset_media_set_id
...
WHERE msdb_dbo_backupset_type = 'D'GROUP BY msdb_dbo_backupset_database_name HAVING (MAX(msdb_dbo_backupset_backup_finish_date) < DATEADD(hh, -24, GETDATE()))
UNION 
...
FROM   master_dbo_sysdatabases LEFT JOIN msdb_dbo_backupset
