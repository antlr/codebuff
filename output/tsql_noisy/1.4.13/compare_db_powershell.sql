...
SELECT @EmployeeList = COALESCE(@EmployeeList + ', ','') + CAST(Emp_UniqueID AS varchar(5)) FROM SalesCallsEmployees
WHERE SalCal_UniqueID = 1


SELECT @dblistData = COALESCE(@dblistData + ', ','') + name FROM sys.databases
WHERE name LIKE 'DMart%Data'
AND name NOT LIKE '%Template%'
GROUP BY name
SELECT @dblistStage = COALESCE(@dblistStage + ', ','') + name FROM sys.databases
...
SELECT @CDCData = COALESCE(@CDCData + ', ','') + name FROM sys.databases
...
SELECT @ServerList = COALESCE(@ServerList + ', ','') + RTRIM(LTRIM([server_name])) FROM Status_dbo.t_server s
...
SELECT @cmd = 'E:\Dexma\powershell_bits\Compare-DbamaintSchema.ps1 ' + '-ServerList ' + @ServerList + ' -Column -Log'
