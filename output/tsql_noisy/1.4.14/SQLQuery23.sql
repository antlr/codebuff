SELECT SourceDB
    ...
    , CONVERT(NVARCHAR(12), DATEADD(ms,
                                    DATEDIFF(ms, (SELECT ErrorDateTime WHERE ErrorMessage LIKE '%Validation phase is beginning.%'), (SELECT ErrorDateTime WHERE ErrorMessage LIKE '%Cleanup phase is beginning.%')), 0), 114) AS TaskTimeTaken
        --, * 
...
FROM dbo.DMartComponentLogging dmcl
     JOIN dbo.DMartComponentLogging dmcl2 ON dmcl.TaskName = dmcl2.TaskName
                                             AND dmcl.SourceServer = dmcl2.SourceServer AND
