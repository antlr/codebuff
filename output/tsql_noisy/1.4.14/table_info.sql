...
SELECT tab.name table_name
    ...
    , CASE WHEN col.isnullable = 1 THEN 'Y'
           ELSE 'N'
      END is_nullable
    , CASE WHEN col.status &80 = 80 THEN 'Y'
           ELSE 'N'
      END is_identity
FROM sysobjects tab,
       syscolumns col
...
WHERE  obj.xtype = 'C'
AND com.id = obj.id
AND tab.id = obj.parent_obj AND
tab.name <> 'dtproperties'
ORDER BY 1,2

/* Get primary key constraints */

SELECT tab.name table_name, ind.name constraint_name, INDEX_COL(tab.name, ind.indid, idk.keyno) column_name, idk.keyno pos
FROM sysobjects tab,
       sysindexes ind
, sysindexkeys idk
...
SELECT tab.name table_name
    ...
    , CASE WHEN ind.status &2 = 2 THEN 'Y'
           ELSE 'N'
      END is_unique
FROM sysindexes ind,
       sysindexkeys idk
